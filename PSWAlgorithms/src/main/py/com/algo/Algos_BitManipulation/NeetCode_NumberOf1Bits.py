...
class Solution:
    ...
    # Bitwise and operation. This will run loop for as many times as there are 1s
    # Beats 96.81% 24ms
    ...
    # Convert to binary and check each digit. This will run loop for each bit in the binary
    # Beats 96.81% 24ms
    def hammingWeight(self, n):
        sum = 0
        # print(n)
    ...
    # Take mod by 2 and count 1s
    # Beats 56.85% 34ms
    def hammingWeight_rightShift(self, n):
        sum = 0
        while n > 0:
            sum += n % 2
            n = n >> 1
    ...
    def hammingWeight_oneLiner(self, n: int) -> int:
        return bin(n).count("1")

    # Beats 56.85% 34 ms
    def hammingWeight_nocheck(self, n):
        sum = 0
        # print(n)
        for val in bin(n)[2:]:
            sum += int(val)
